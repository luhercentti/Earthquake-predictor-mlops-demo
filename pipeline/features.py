...
FEATURE_COLS = [
    ...
    "count_7d", "mean_mag_7d", "max_mag_7d", "std_mag_7d", "mean_depth_7d",
    "count_30d", "mean_mag_30d", "max_mag_30d", "std_mag_30d", "mean_depth_30d",
    "count_90d", "mean_mag_90d", "max_mag_90d", "std_mag_90d", "mean_depth_90d",
    "count_365d", "mean_mag_365d", "max_mag_365d", "std_mag_365d", "mean_depth_365d",
    # current event features
    "magnitude", "depth",
    "days_since_last",      # inter-event time from previous event in same cell
    "mag_last",             # magnitude of the previous event in cell
...
def _cell_features(g: pd.DataFrame) -> pd.DataFrame:
    ...
    # DatetimeIndex needed for time-offset rolling
    g = g.set_index("time")

    for window, label in [("7D", "7d"), ("30D", "30d"),
                           ("90D", "90d"), ("365D", "365d")]:
        # closed='left' = window is [t-w, t) → past events only, excludes current
    ...
    # inter-event time (seconds → days)
    idx_series = g.index.to_series()
    g["days_since_last"] = idx_series.diff().dt.total_seconds().div(86400).fillna(999.0)

    # previous event magnitude (0 when no prior event in cell)
    g["mag_last"] = g["magnitude"].shift(1).fillna(0.0)

    # labels: next event in same cell
